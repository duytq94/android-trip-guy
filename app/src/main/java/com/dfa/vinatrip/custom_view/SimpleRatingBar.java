...
public class SimpleRatingBar extends View {
    ...
    public enum Gravity {
        ...
        static Gravity fromId(int id) {
            ...
            // default value
            Log.w("SimpleRatingBar", String.format("Gravity chosen is neither 'left' nor 'right', " +
                    "I will set it to Left"));
            return Left;
        }
    }
    ...
    // Internal variables
    private float currentStarSize = 38f;
    ...
    private void parseAttrs(AttributeSet attrs) {
        ...
        borderColor = arr.getColor(R.styleable.SimpleRatingBar_srb_borderColor,
                getResources().getColor(R.color.golden_stars));
        ...
        pressedStarBackgroundColor = arr.getColor(R.styleable.SimpleRatingBar_srb_pressedStarBackgroundColor,
                starBackgroundColor);
        ...
        starsSeparation = arr.getDimensionPixelSize(R.styleable.SimpleRatingBar_srb_starsSeparation,
    ...
    private void validateAttrs() {
        if (numberOfStars <= 0) {
            throw new IllegalArgumentException(String.format("SimpleRatingBar initialized with invalid value " +
        ...
        if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
                > maxStarSize) {
            Log.w("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater " +
                    "than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
        }
        if (stepSize <= 0) {
            throw new IllegalArgumentException(String.format("SimpleRatingBar initialized with invalid value " +
    ...
    private void drawStar(Canvas canvas, float x, float y, float filled, Gravity gravity) {
        ...
        // Note: below, currentStarSize*0.02f is a minor correction so the user won't see a vertical black
        // line in between the fill and empty color
        if (gravity == Gravity.Left) {
            // color star fill
            ...
            // color star fill
            canvas.drawRect(x + currentStarSize - (fill + currentStarSize * 0.02f), y, x + currentStarSize,
                    y + currentStarSize, paintStarFill);
            // draw star background
  ...
  /* ----------- GETTERS AND SETTERS ----------- */
    ...
    public void setStarSize(float starSize) {
        ...
        if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
            Log.w("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater " +
                    "than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
        }
        // force re-calculating the layout dimension
        requestLayout();
        generateInternalCanvas(getWidth(), getHeight());
    ...
    private float normalizeRating(float rating) {
        if (rating < 0) {
            Log.w("SimpleRatingBar", String.format("Assigned rating is less than 0 (%f < 0), " +
                    "I will set it to exactly 0", rating));
            return 0;
        } else if (rating > numberOfStars) {
            Log.w("SimpleRatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), " +
                    "I will set it to exactly numberOfStars", rating, numberOfStars));
            return numberOfStars;
        } else {
            return rating;
        }
